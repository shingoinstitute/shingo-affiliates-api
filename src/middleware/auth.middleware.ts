...
export class AuthMiddleware implements NestMiddleware {
  ...
  public resolve(level: 1 | 2, resource?: string): MiddlewareFunction {
    return (req, res, next) => {
      let isAfMan =
        req.session.user && req.session.user.role.name === 'Affiliate Manager'
      if (isAfMan) return next && next()

      const realResource = this.parseResource(
        resource && resource.match(/^.*\s--\s$/)
          ? resource + req.session.affiliate
          : !resource
          ? (resource = `${req.path}`)
          : resource
      )
      ...
      return this.authService
        .canAccess(realResource, level, req.headers['x-jwt'])
        .then(result => {
          if (result && result.response) return next && next()
          throw {
            error: 'ACCESS_FORBIDDEN',
            message: `Insufficent permission to access ${realResource} at level ${level} by user: ${
              req.session.user ? req.session.user.Email : 'anonymous'
            }`,
          }
        })
        .catch(error => {
