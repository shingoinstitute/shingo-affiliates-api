...
import SalesforceService, {
  salesforceServiceFactory,
} from '../components/salesforce/new-salesforce.component'
...
export class IsValidMiddleware implements NestMiddleware {
  ...
  public resolve(): MiddlewareFunction {
    return (req, res, next) => {
      ...
      if (
        req.path === '/workshops' &&
        (req.query.isPublic || req.headers['x-is-public'])
      )
        return next && next()
      if (!req.headers['x-jwt'] && !req.session.user)
        return res
          .status(HttpStatus.BAD_REQUEST)
          .json({ error: 'INVALID_TOKEN', header: 'x-jwt' })
      ...
      return this.authService
        ...
        .then(user => {
          ...
          req.session.user.role = user.roles.map(
            (role: { service: string }) => {
          ...
          return this.sfService.retrieve<Contact>({
            object: 'Contact',
            ids: [user.extId],
          })
        })
        .then(response => {
          ...
          req.session.user = _.merge(
            contact,
            _.omit(req.session.user, ['email'])
          )
