...
export class WorkshopsController {
    ...
    public async read( @Response() res, @Param('id') id): Promise<Response> {
        // Check the id
        if (!id.matches(/a[\w\d]{14,17}/)) return this.handleError(res, 'Error in WorkshopsController.read(): ', { error: 'INVALID_SF_ID', message: `${id} is not a valid Salesforce ID.` }, HttpStatus.BAD_REQUEST);
    ...
    public async facilitators( @Response() res, @Param('id') id): Promise<Response> {
        ...
        // Check the id
        if (!id.matches(/a[\w\d]{14,17}/)) this.handleError(res, 'Error in WorkshopsController.facilitators(): ', { error: 'INVALID_SF_ID', message: `${id} is not a valid Salesforce ID.` }, HttpStatus.BAD_REQUEST);
    ...
    public async create( @Response() res, @Body() body, @Session() session): Promise<Response> {
        ...
        if (!body.Organizing_Affiliate__c.matches(/[\w\d]{15,17}/)) return this.handleError(res, 'Error in WorkshopsController.create(): ', { error: 'INVALID_SF_ID', message: `${body.Organizing_Affiliate__c} is not a valid Salesforce ID.` }, HttpStatus.BAD_REQUEST);
