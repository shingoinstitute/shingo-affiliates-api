...
export class WorkshopsController extends BaseController {
  ...
  public async readAll(@Response() res: Res, @Session() session: any) {
    ...
    try {
      const workshops = await this.workshopsService.getAll(
        false,
        true,
        session.user
      )
  ...
  public async readPublic(
    ...
    try {
      ...
      return this.handleError(
        res,
        'Error in WorkshopsController.readPublic(): ',
        error
      )
    }
  }
  ...
  public async describe(
    ...
    try {
      const describeObject = await this.workshopsService.describe(
        refresh === 'true'
      )
      ...
      return this.handleError(
        res,
        'Error in WorkshopsController.describe(): ',
        error
      )
    }
  }
  ...
  public async search(
    ...
    try {
      const workshops = await this.workshopsService.search(
        search,
        retrieve,
        refresh === 'true'
      )
  ...
  public async facilitators(@Response() res: Res, @Param('id') id: string) {
    ...
    if (!id.match(/a[\w\d]{14,17}/))
      return this.handleError(
        res,
        'Error in WorkshopsController.facilitators(): ',
        {
          error: 'INVALID_SF_ID',
  ...
  public async create(
    ...
    if (
      session.user.role.name !== 'Affiliate Manager' &&
      ...
      return this.handleError(
        ...
        {
          error: 'PERM_DENIDED',
          message: `You are not allowed to post workshops for the Affiliate with Id ${
            body.Organizing_Affiliate__c
          }`,
        },
        HttpStatus.FORBIDDEN
    ...
    try {
      ...
      session.user.permissions.push({
        resource: `/workshops/${sfSuccess.id}`,
        level: 2,
      })
  ...
  public async update(
    ...
    if (
      ...
      return this.handleError(
        ...
        {
          error: 'INVALID_SF_ID',
          message: `${body.Organizing_Affiliate__c} or ${id} or ${
            body.Id
          } is not a valid Salesforce ID.`,
        },
        HttpStatus.BAD_REQUEST
    ...
    if (
      ...
      return this.handleError(
        ...
        {
          error: 'PERM_DENIDED',
          message: `You are not allowed to update workshops for the Affiliate with Id ${
            body.Organizing_Affiliate__c
          }`,
        },
        HttpStatus.FORBIDDEN
  ...
  public async uploadAttendeeFile(
    @Request() req: Req,
    @Response() res: Res,
    @Param('id') id: string
  ) {
    if (!id.match(/a[\w\d]{14,17}/))
      return this.handleError(
        res,
        'Error in WorkshopsController.uploadAttendeeFile(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
  ...
  public async uploadEvaluations(
    @Request() req: Req,
    @Response() res: Res,
    @Param('id') id: string
  ) {
    if (!id.match(/a[\w\d]{14,17}/))
      return this.handleError(
        res,
        'Error in WorkshopsController.uploadEvaluations(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
    ...
    return upload(req, res, error => {
      ...
      try {
        const files = (req as any).files.map(
          (file: { buffer: { toString: (arg0: string) => void } }) => {
  ...
  public async delete(@Response() res: Res, @Param('id') id: string) {
    ...
    if (!id.match(/a[\w\d]{14,17}/))
      return this.handleError(
        res,
        'Error in WorkshopsController.delete(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
  ...
  public async cancel(
    ...
    if (!id.match(/a[\w\d]{14,17}/))
      return this.handleError(
        res,
        'Error in WorkshopsController.cancel(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
