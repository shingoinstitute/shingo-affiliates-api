...
export class FacilitatorsController extends BaseController {
  ...
  public async readAll(
    ...
    if (!isAfMan && !session.affiliate)
      return this.handleError(
        res,
        'Error in FacilitatorsController.readAll(): ',
        { error: 'MISSING_FIELDS' },
        HttpStatus.FORBIDDEN
      )

    try {
      const facilitators = await this.facilitatorsService.getAll(
        true,
        isAfMan ? xAffiliate : session.affiliate
      )
  ...
  public async describe(
    ...
    try {
      const describeObject = await this.facilitatorsService.describe(
        refresh === 'true'
      )
  ...
  public async search(
    ...
    if (!isAfMan && !session.affiliate)
      return this.handleError(
        res,
        'Error in FacilitatorsController.search(): ',
    ...
    try {
      const searchRecords = await this.facilitatorsService.search(
        search,
        retrieve,
        isMapped === 'true',
        isAfMan ? '' : session.affiliate,
        refresh === 'true'
      )
  ...
  public async read(@Response() res: Res, @Param('id') id: string) {
    ...
    if (!id.match(/[\w\d]{15,18}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.read(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
        HttpStatus.BAD_REQUEST
      )
  ...
  public async resetPassword(
    ...
    try {
      ...
      return this.handleError(
        res,
        'Error in FacilitatorsController.resetPassword(): ',
        error
      )
    }
  }
  ...
  public async changePassword(@Response() res: Res, @Body() body: any) {
    const valid = checkRequired(body, ['password', 'token'])
    if (!valid.valid)
      return this.handleError(
        res,
        'Error in FacilitatorsController.changePassword(): ',
        { error: 'MISSING_FIELDS', fields: valid.missing },
        HttpStatus.BAD_REQUEST
      )

    try {
      const user = await this.facilitatorsService.resetPassword(
        body.token,
        body.password
      )
  ...
  public async create(@Response() res: Res, @Body() body: any) {
    const required = checkRequired(body, [
      'AccountId',
      'FirstName',
      'LastName',
      'Email',
    ...
    if (!required.valid)
      return this.handleError(
        res,
        'Error in FacilitatorsController.create(): ',
        { error: 'MISSING_FIELDS', fields: required.missing },
        HttpStatus.BAD_REQUEST
      )
    ...
    if (!body.AccountId.match(/[\w\d]{15,17}/))
      return this.handleError(
        ...
        {
          error: 'INVALID_SF_ID',
          message: `${body.AccountId} is not a valid Salesforce ID.`,
        },
        HttpStatus.BAD_REQUEST
      )
    ...
    try {
      ...
      await this.mailer.sendMail({
        to:
          process.env.NODE_ENV === 'development'
            ? 'dustin.e.homan@usu.edu,craig.blackburn@usu.edu'
        ...
        html: `A new account has been created for ${result.id}:${body.Email}:${
          body.password
        }.`,
      })
      ...
      await this.mailer.sendMail({
        ...
        html: `Hello ${body.FirstName} ${
          body.LastName
        },<br><br>Your account for the Shingo Affiliate Portal has been created!<br><br>Your temporary password is:<br>&emsp;${
          body.password
        }<br>Please change it when you first log in.<br><br>Thank you,<br><br>Shingo Institute`,
      })
  ...
  public async map(
    ...
    if (!id.match(/[\w\d]{15,18}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.map(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
  ...
  public async update(
    ...
    if (!id.match(/[\w\d]{15,17}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.update(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
    ...
    if (!body.Id || body.Id !== id)
      return this.handleError(
        res,
        'Error in FacilitatorsController.update(): ',
        { error: 'MISSING_FIELDS', fields: ['Id'] },
        HttpStatus.BAD_REQUEST
      )
  ...
  public async delete(
    ...
    if (!id.match(/[\w\d]{15,17}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.delete(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
    ...
    try {
      ...
      let deleted = false
      if (deleteAuth === 'true')
        deleted = await this.facilitatorsService.deleteAuth(id)
      else await this.facilitatorsService.unmapAuth(id)

      return res
        .status(HttpStatus.OK)
        .json({ salesforce: true, auth: deleted, record })
    } catch (error) {
      return this.handleError(
        res,
        'Error in FacilitatorsController.delete(): ',
   ...
   */
  @Delete('/:id/login')
  public async deleteLogin(@Response() res: Res, @Param('id') id: string) {
    if (!id.match(/[\w\d]{15,17}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.deleteLogin(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
   ...
   */
  @Delete('/:id/unmap')
  public async unmap(@Response() res: Res, @Param('id') id: string) {
    if (!id.match(/[\w\d]{15,17}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.unmap(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
  ...
  public async changeRole(
    ...
    if (!id.match(/[\w\d]{15,17}/))
      return this.handleError(
        res,
        'Error in FacilitatorsController.changeRole(): ',
        {
          error: 'INVALID_SF_ID',
          message: `${id} is not a valid Salesforce ID.`,
        },
