...
export class AuthController extends BaseController {
  ...
  public async login(
    ...
    try {
      ...
      return res
        ...
        .json(
          _.omit((req as any).session.user, [
            'permissions',
            'extId',
            'services',
            'role.permissions',
  ...
  private async getSessionUser(user: any) {
    const [contact] = await this.sfService.retrieve<Contact>({
      object: 'Contact',
      ids: [user.extId],
    })
    let sessionUser = _.omit(user, ['password', 'roles'])
    sessionUser = _.merge(contact, _.omit(sessionUser, ['email']))
    sessionUser.role = user.roles.map((role: { service: string }) => {
      if (role.service === 'affiliate-portal')
        return _.omit(role, ['users', 'service'])
    })[0]
  ...
  public async logout(@Request() req: Req, @Response() res: Res) {
    if (!(req as any).session.user)
      return this.handleError(
        res,
        'Error in AuthController.logout(): ',
    ...
    try {
      ...
      let user = await this.authService.updateUser(_.pick(
  ...
  public async changePassword(
    ...
    if (!body.password)
      return this.handleError(
        res,
        'Error in AuthController.changePassword(): ',
        { error: 'MISSING_FIELDS', fields: ['password'] },
        HttpStatus.BAD_REQUEST
      )
    ...
    try {
      ...
      const updated = await this.authService.updateUser(_.pick(
        (req as any).session.user,
        ['id', 'password']
      ) as any)

      ;(req as any).session.user = await this.authService.getUser(
        `user.id=${(req as any).session.user.id}`
      )
      ;(req as any).session.user = await this.getSessionUser(
        (req as any).session.user
      )

      return res
        .status(HttpStatus.OK)
        .json({ jwt: (req as any).session.user.jwt })
    } catch (error) {
      return this.handleError(
        res,
        'Error in AuthController.changePassword',
  ...
  public async loginAs(
    ...
    if (!body.adminId || !body.userId)
      return this.handleError(
        res,
        'Error in AuthController.loginAs(): ',
        { error: 'MISSING_FIELDS', fields: ['adminId', 'userId'] },
        HttpStatus.BAD_REQUEST
      )
    if ((req as any).session.user.id != body.adminId)
      return this.handleError(
        res,
        'Error in AuthController.loginAs(): ',
        { error: 'UNAUTHORIZED' },
    ...
    try {
      const user = await this.authService.loginAs({
