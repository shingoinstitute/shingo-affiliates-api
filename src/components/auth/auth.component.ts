...
const authservices = grpc.load(
  path.join(__dirname, '../../../proto/auth_services.proto')
).authservices
...
export class AuthService {
  ...
  private getClient() {
    ...
    const endpoint =
      envendpoint.indexOf(':') !== -1 ? envendpoint : `${envendpoint}:80`
    return new authservices.AuthServices(
      endpoint,
   ...
   * @memberof AuthService
   */
  public createPermission(permission: {
    resource: string
    level: number
  }): Promise<any> {
   ...
   * @memberof AuthService
   */
  public grantPermissionToUser(
    resource: string,
    level: 0 | 1 | 2,
    userId: number
  ): Promise<any> {
    return this.client.grantPermissionToUserAsync({
      resource,
      level,
      accessorId: userId,
    })
  ...
  public grantPermissionToRole(
    ...
    return this.client.grantPermissionToRoleAsync({
      resource,
      level,
      accessorId: roleId,
    })
   ...
   * @memberof AuthService
   */
  public revokePermissionFromUser(
    resource: string,
    level: 0 | 1 | 2,
    userId: number
  ): Promise<any> {
    return this.client.revokePermissionFromUserAsync({
      resource,
      level,
      accessorId: userId,
    })
  ...
  public revokePermissionFromRole(
    ...
    return this.client.revokePermissionFromRoleAsync({
      resource,
      level,
      accessorId: roleId,
    })
